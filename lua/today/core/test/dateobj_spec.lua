...
local DateObj = dates.DateObj
    ...
    describe("DateObj", function()
        ...
        describe("less than", function()
            ...
            it(
                "should return unequal when one is infinite and one is finite, reversed",
                function()
                    local ds1 = DateObj:from_ymd(2021, 10, 2)
                    local ds2 = DateObj:infinite_future()
                    assert.is.falsy(ds2 < ds1)
                end
            )
        end)
    ...
    describe("weeks_until", function()
        -- 2021-07-04 was a sunday

        it("returns zero for current sunday", function()
            local date = DateObj:new("2021-07-04")
            assert.are.equal(date:weeks_until(DateObj:new("2021-07-04")), 0)
        end)

        it("returns zero for monday", function()
            local date = DateObj:new("2021-07-04")
            assert.are.equal(date:weeks_until(DateObj:new("2021-07-05")), 0)
        end)

        it("returns zero for saturday", function()
            local date = DateObj:new("2021-07-04")
            assert.are.equal(date:weeks_until(DateObj:new("2021-07-10")), 0)
        end)

        it("returns one for next sunday", function()
            local date = DateObj:new("2021-07-04")
            assert.are.equal(date:weeks_until(DateObj:new("2021-07-11")), 1)
        end)

        it("returns one for next monday", function()
            local date = DateObj:new("2021-07-04")
            assert.are.equal(date:weeks_until(DateObj:new("2021-07-12")), 1)
        end)

        it("returns math.huge for infinite future", function()
            local date = DateObj:new("2021-07-04")
            assert.are.equal(date:weeks_until(DateObj:new("infinite_future")), math.huge)
        end)
    end)
